...
scp second-brain-backup.sql user@deploy-host.local:~/
scp ~/.config/cliproxy-auth.json user@deploy-host.local:~/
```
...
minikube mount \
  --uid=10001 \
  --gid=10001 \
  "~/Google Drive/공유 드라이브/<shared-drive>:/mnt/drive" &
```
...
<dict>
  ...
  <array>
    ...
    <string>/Users/username/Google Drive/공유 드라이브/<shared-drive>:/mnt/drive</string>
  </array>
  <key>RunAtLoad</key>
  <true/>
  <key>KeepAlive</key>
...
`~/Library/LaunchAgents/<portforward>.plist`:
