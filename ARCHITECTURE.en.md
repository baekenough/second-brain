...
The command `minikube mount --uid=10001 --gid=10001 ~/Google\ Drive/Shared\ Drive/<shared-drive>:/mnt/drive` exposes the host Drive folder as a ReadOnlyMany hostPath PV (100 Gi) mounted at `/data/drive` inside the Pod.
...
### Frontend (`<web-app>` Next.js service)
...
| `<web-app>:dev` | `node:22-alpine` (standalone) | ~195 MB | 10001 |
