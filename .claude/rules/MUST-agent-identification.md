...
| 컨텍스트 컴팩션 직후 재개 턴 | 요약이 규칙 본문을 대체한 것처럼 느껴지고, "서두 없이 바로 이어가라"는 재개 지시를 "헤더 없이"로 확대 해석하기 쉬움 |

**자가 점검**: 위 구간에서는 응답을 시작하기 전에 헤더 유무를 명시적으로 확인한다. 알림으로 턴이 시작되면 규칙이 리셋된 것처럼 느껴지지만 그렇지 않다 — 알림 턴도 사용자 입력 턴과 동일하게 R007이 적용된다.

**컴팩션 재개 판별 기준**: 재개 지시가 금지하는 것은 *요약에 대한 언급*("이어서 하겠습니다", "요약을 확인했습니다" 같은 메타 서두)이지 *식별 헤더*가 아니다. R007 헤더는 서두가 아니라 응답의 구조적 일부이므로 어떤 재개 지시로도 면제되지 않는다.

**재발 이력**: 2026-08-18 세션에서 동일 세션 내 2회 누락(모두 사용자가 `@CLAUDE.md 재주입`으로 지적). 1차 발생 시 "나중에 규칙을 강화하겠다"고 미룬 것이 2차 재발의 원인이었다 — R016 안티패턴("나중에 고치겠다")의 실사례. 2026-08-26 세션에서는 컨텍스트 컴팩션 직후 재개 턴에 R007 헤더가 3회째 누락됐다 — 앞선 2회와 달리 사용자 입력 턴이 아니라 **자동 재개 턴**에서 발생했다. 직접 원인은 재개 시 시스템이 붙이는 "요약을 언급하지 말고 서두 없이 이어가라"는 지시를 헤더까지 생략하라는 뜻으로 확대 적용한 것이었다.
