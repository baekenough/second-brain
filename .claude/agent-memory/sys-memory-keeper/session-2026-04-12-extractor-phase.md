...
- **Why**: Google Drive는 로컬 sync 폴더(~/Google Drive/공유 드라이브/<shared-drive>)를 FS로 스캔하되, .gdoc 등 네이티브 파일은 Drive API export 필요
...
- 바이너리: `/tmp/<old-repo>-server`
- PID: 28364
- 포트: :9200
- 로그: `/tmp/<old-repo>.log`
- COLLECT_INTERVAL: 5m (.env 변경, 미커밋)
- DB: docker `<old-repo>-postgres-1` (pgvector/pg16, user=brain, db=<old-db>)
