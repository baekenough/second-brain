...
**Current state**: Collection jobs are scheduled with `robfig/cron` inside the Go server process. All collectors share the same process lifecycle and have no dependency graph.
...
2. The Go HTTP server becomes API-only (no scheduler). Remove `robfig/cron` dependency.
...
**Current state**: `pgvector` stores 1536-dimension OpenAI-compatible embeddings. Embedding is optional; the system falls back to PostgreSQL full-text search when `EMBEDDING_API_URL` is unset.
...
- Helm chart with separate deployments for server and web.
