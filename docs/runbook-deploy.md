...
> **대상 서버**: `<deploy-host>` (minikube + docker driver)
> **SSH alias**: `host24_home_server-ext`
> **외부 URL**: `https://your-domain.example`
> **네임스페이스**: `second-brain`
> **책임자**: @<maintainer> (or rotation)
> **최종 갱신**: 2026-04-14
...
| 항목 | 확인 방법 | 기대 결과 |
|------|-----------|-----------|
| SSH alias 설정 | `ssh host24_home_server-ext echo ok` | `ok` 출력 |
| minikube 실행 중 | `ssh host24_home_server-ext minikube status` | `host: Running` |
| cliproxy 인증 파일 존재 | `ssh host24_home_server-ext ls ~/.cli-proxy-api/` | `codex-*.json` 파일 확인 |
...
로컬 머신
  └── git tag → git push origin <tag>
        │
        ▼
<deploy-host> (minikube + docker driver)
  ...
  └── nginx reverse proxy → https://your-domain.example
```
