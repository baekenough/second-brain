...
**중요**: 프로덕션 `docker-compose.ubuntu1.yml`은 이 사본과 별도로 관리된다.
배포(이 사본으로 프로덕션 파일을 덮어쓰는 작업) 전에는 반드시 서비스 목록을
대조할 것 — 현재 6개: `server`, `collector`, `mcp`, `eval-runner`, `web`,
`neo4j`. (2026-08-29 배포 때 이 사본에 `mcp`/`eval-runner` 정의가 없어서
덮어쓰기로 프로덕션에서 두 서비스가 통째로 사라졌다가 3주 뒤에야 발견됐다 —
컨테이너는 재생성되지 않아 계속 떠 있었기 때문이다. 2026-09-19 백업에서
복원 + 이 사본에도 반영. 자세한 내용은 아래 "함정 6" 참고.)
...
### 6. 리포 사본에서 서비스 정의 유실 — 2026-09-19

**사고**: 2026-08-29 배포(next-auth 제거) 때 이 사본(`docker-compose.yml`)으로
프로덕션 `docker-compose.ubuntu1.yml`을 덮어썼는데, 당시 이 사본에는 `mcp`와
`eval-runner` 서비스 정의가 빠져 있었다(README "구성" 섹션은 두 서비스가
운영 중이라고 정확히 기록하고 있었지만, compose 파일 자체는 갱신되지 않은
상태였다). 덮어쓰기로 프로덕션 파일에서도 두 서비스 정의가 사라졌다.

**왜 즉시 안 드러났는가**: `docker compose up -d`는 이미 떠 있는 컨테이너를
재생성하지 않으므로, 정의가 없어져도 기존 `mcp`/`eval-runner` 컨테이너는
그대로 계속 실행됐다. 3주 뒤(2026-09-19) `docker compose up -d mcp`를
실행하려다 "no such service"로 실패하고 나서야 발견됐다.

**해결**: 프로덕션 파일은 배포 시점 백업(`bak-20260829`)에서 두 서비스
정의를 복원했다. 이 사본에도 동일하게 반영했다(위 compose 파일 참고).

**지침**: 이 사본으로 프로덕션 파일을 덮어쓰기 전에 반드시
`docker compose -f docker-compose.ubuntu1.yml config --services`를 (있다면
배포 전 백업 대상 파일 기준으로) 실행해 서비스 목록이 이 README "구성"
섹션 및 위 서비스 6개와 일치하는지 대조할 것. README를 갱신하는 것만으로는
compose 파일 자체의 드리프트를 막지 못한다는 것이 이번 사고의 교훈이다.
