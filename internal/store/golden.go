...
func (s *GoldenStore) UpsertQueryByText(ctx context.Context, text, source string) (uuid.UUID, error) {
	...
	if id, found, err := s.findQueryByNormalizedText(ctx, norm); err != nil {
		return uuid.Nil, err
	} else if found {
		return id, nil
	}
	...
	if err != pgx.ErrNoRows {
		return uuid.Nil, fmt.Errorf("golden: insert query: %w", err)
	}

	// Exact-text race: another writer inserted this identical text between
	// our normalized lookup above and this INSERT. Resolve by exact text
	// rather than failing a request that would succeed on retry anyway.
	if err := s.pg.pool.QueryRow(ctx,
		`SELECT id FROM golden_queries WHERE text = $1`, text).Scan(&id); err != nil {
		return uuid.Nil, fmt.Errorf("golden: resolve race-inserted query: %w", err)
	}
	return id, nil
}
