// Package chunkctx builds the deterministic context header that gets
// prepended to a chunk's text before it is used as either an embedding input
// (internal/scheduler) or a sparse-search match target (internal/search,
// internal/store — issue #270).
//
// This package used to live inside internal/scheduler
// (BuildChunkContextHeader), where only the embedding pipeline could reach
// it. #270 needs the SAME header logic from internal/search/internal/store,
// and a search -> scheduler import is not viable (scheduler already depends
// on internal/search's sibling packages for the collection pipeline), so the
// pure, side-effect-free header builder was extracted here. This move changed
// nothing about the builder's behaviour — see chunkctx_test.go's golden
// table, which pins the exact output for representative documents and must
